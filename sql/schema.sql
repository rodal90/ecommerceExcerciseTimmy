...
create table PROVIDER (id bigint generated by default as identity, nif VARCHAR(9) CHECK(LENGTH(NIF) >= 2), phone CHAR(18) CHECK(LENGTH(PHONE) >= 9) not null, cif VARCHAR(30) CHECK(LENGTH(CIF) >= 9), name VARCHAR(150) CHECK(LENGTH(NAME) >= 1) not null, email varchar(255), primary key (id));
